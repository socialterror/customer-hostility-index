...
MIN_ORDER=157; MAX_ORDER=184
PHRASES=['price increase','pricing action','pricing actions','higher price','higher prices','higher asp','average selling price','average price','rate increase','rate increases','subscription','subscriptions','renewal','renewals','seat','seats','license','licenses','licensing','usage','consumption','premium','tier','tiers','bundle','bundling','add-on','upsell','cross-sell','revenue per customer','revenue per user','arpu','discontinue','discontinuation','sunset','phase out','migration','migrate','portfolio','depriorit','retire','unauthorized use','piracy','freemium','maintenance','support fee','service fee']
rows=[]
for order in range(MIN_ORDER,MAX_ORDER+1):
 ...
 for line in p.read_text(encoding='utf-8').splitlines():
  if line.startswith('## '): filing=line.split('|')[0].replace('##','').strip()
   ...
   if matched:
    score=len(matched)
    for x in ['price increase','pricing action','higher price','subscription','renewal','license','usage','consumption','premium','tier','bundle','add-on','upsell','cross-sell','discontinue','migration','migrate','unauthorized use','piracy','service fee']:
     if x in lo: score+=3
