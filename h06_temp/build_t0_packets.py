...
def passages(txt):
    ...
    for fam,ks in KEYS.items():
        ...
        if len(hits)>36:
            idx=sorted(set(round(i*(len(hits)-1)/35) for i in range(36))); hits=[hits[i] for i in idx]
        out.append((fam,hits))
    return out
def main():
    ...
    for order in sorted(by):
        rs=sorted(by[order],key=lambda x:x['filing_position']); ticker=rs[0]['ticker']
        ...
        for r in rs:
            ...
            summary.append({'sample_order':order,'ticker':ticker,'filing_position':r['filing_position'],'accession':r['accession_number'],'candidate_passages':total}); time.sleep(.12)
